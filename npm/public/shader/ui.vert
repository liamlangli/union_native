...
highp vec4 fetch_primitive_buffer(const uint pixel_index) {
...
vec4 decode_color(highp uint c)
{
    return vec4(float((c >> 24u) & 0xffu) / 255.0, float((c >> 16u) & 0xffu) / 255.0, float((c >> 8u) & 0xffu) / 255.0, float(c & 0xffu) / 255.0);
}
...
void main()
{
    ...
    if ((vertex_id & GLYPH_MASK) != 0u) {
        // draw glyph
        highp uint corner_id = decode_corner_idx(vertex_id);
        highp uint header_offset = decode_header_offset(vertex_id);
        highp uint header_ptr = ptr - header_offset - 1u;
        ...
        highp uint font_ptr = floatBitsToUint(glyph_header.z);
        vec4 font_data = fetch_primitive_buffer(font_ptr);
        highp uint glyph_ptr = font_ptr + 2u + floatBitsToUint(glyph_data.y) * 2u;
        ...
        if (type == UI_PRIM_RECTANGLE) {
            ...
            color = decode_color(floatBitsToUint(rect_data.x));
            highp uint raw_clip = uint(rect_data.y);
            ...
            color = decode_color(floatBitsToUint(primitive_data.z));
            highp uint raw_clip = floatBitsToUint(primitive_data.w);
            clip = raw_clip == 0u ? 0u : ptr - raw_clip;
            ...
            type = floatBitsToUint(sample_data.z);
        }
    }
