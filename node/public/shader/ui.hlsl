...
Texture2D primitive_buffer: register(t0);
SamplerState linear_sampler: register(s0);

vertex_output verrex_main(vertex_data _in) {
    vertex_output out;

    uint vertex_id = _in.vertex_id;
...
float4 fragment_main(vertex_output _in)
{
    if (_in.clip != 0u && !contains(_in.screen_point, _in.clip_rect)) discard_fragment();
    sampler linear_sampler(mag_filter::linear, min_filter::linear);
    if (_in.type == UI_PRIM_GLYPH) {
        float3 msdf = font_texture.sample(linear_sampler, _in.sample_point).xyz;
        float sd = median(msdf.r, msdf.g, msdf.b);
        float w = fwidth(sd);
        float opacity = smoothstep(0.5 - w, 0.5 + w, sd);
        return float4(_in.color.xyz, opacity);
    } else if (_in.type == UI_PRIM_TRIANGLE_ADVANCED_ICON) {
        return icon_texture.sample(linear_sampler, _in.sample_point);
    } else {
        return _in.color;
    }
}
