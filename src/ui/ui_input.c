...
void ui_input_render_cursor(ui_input_t *input, ui_rect rect, u32 clip) {
    ...
    fill_rect(0, cursor_style, cursor_rect, clip);
}

void ui_input_handle_edit(ui_input_t *input) {
    const bool control_pressed =
        ui_state_is_key_pressed(KEY_LEFT_CONTROL) || ui_state_is_key_pressed(KEY_RIGHT_CONTROL) ||
        ui_state_is_key_pressed(KEY_LEFT_SUPER) || ui_state_is_key_pressed(KEY_RIGHT_SUPER);
    ...
    if (ui_state_is_key_press(KEY_BACKSPACE)) {
        ...
        ui_state_delete_key_press(KEY_BACKSPACE);
...
bool ui_input(ui_input_t *input, ui_style style, ui_rect rect, u32 layer_index, u32 clip) {
    bool result = false;
    ...
    if (active || focus) {
        ...
        ui_input_handle_edit(input);
        if (ui_state_is_key_press(KEY_ENTER)) {
            result = true;
    ...
    if (active) {
        ...
        if (!ui_state_hovering(rect, layer_index) && state->left_mouse_press) {
            ui_state_clear_active();
            ui_state_clear_focus();
            input->label.render_selected = false;
        }
    ...
    if (active || focus) {
        ui_input_render_cursor(input, rect, clip);
    }
