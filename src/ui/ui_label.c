...
void ui_label_compute_size_and_offset(ui_label_t *label) {
    ...
    label->cursor_index = MACRO_MIN(label->cursor_index, (int)label->text.size());
    label->start_index = MACRO_MIN(label->start_index, (int)label->text.size());
}
...
u32 ui_label_locate_cursor(ui_label_t *label, ui_rect rect, float2 location) {
    ...
    int index = 0;
    while (index < (int)label->text.size()) {
...
f32 ui_label_offset_at(ui_label_t *label, int index) {
    ...
    int text_length = (int)label->text.size();
